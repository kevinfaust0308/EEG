...
if final_run:
    ...
    # TODO: we will cycle through each contact.
    contact_indices = [(run_id % len(contacts)) - 1]

    # TODO: chosen by nebras as good values
    stft_nperseg = 2048
    stft_noverlap = 1800
...
if CONTACT_MODE:
    X_full, contacts_to_use, regions_to_use = select_contacts2(X_full, contacts, regions, indices_to_use=contact_indices)
...
for step in range(0, num_batches):
    ...
    testData, testTarget,\
        trainShift, validShift, testShift = \
    ...
    if False:
        ...
        trainData, trainTarget, trainShift = augment_dataset_shift(
            trainData, trainTarget, trainShift, np.percentile(trainTarget, percentile), data_range, data_avg_points, shift_by=5, left_shifts=5, right_shifts=5)
    ...
    trainData, trainTarget, trainShift = upsample_bins(trainData, trainTarget, trainShift, num_batches)

    ### TODO:
    if mode_frequency:

        f, t, trainDataSTFT = signal.stft(trainData, 2048, nperseg=stft_nperseg, noverlap=stft_noverlap)
    ...
    trainShift = np.expand_dims(trainShift, axis=-1)
    ...
    trainDatasetInputs = tf.data.Dataset.from_tensor_slices((trainData, trainShift))
    trainDatasetOutputs = tf.data.Dataset.from_tensor_slices(trainTarget)
    trainDataset = tf.data.Dataset.zip((trainDatasetInputs, trainDatasetOutputs)).repeat().batch(BATCH_SIZE).prefetch(tf.data.experimental.AUTOTUNE)
    ...
    if not final_run:
        ...
        # TODO:
        if mode_frequency:
            f, t, validDataFreq = signal.stft(validData, 2048, nperseg=stft_nperseg, noverlap=stft_noverlap)
            fs_ind = next(i for i, res in enumerate(f > fs_cap) if res) if fs_cap else None
        ...
        validShift = np.expand_dims(validShift, axis=-1)
        ...
        validDatasetInputs = tf.data.Dataset.from_tensor_slices((validData, validShift))
        validDatasetOutputs = tf.data.Dataset.from_tensor_slices(validTarget)
        validDataset = tf.data.Dataset.zip((validDatasetInputs, validDatasetOutputs)).batch(len(validData)).prefetch(tf.data.experimental.AUTOTUNE)
        ...
        if USE_DATASET_API:
            ...
            history = model.fit(x=[trainData, trainShift], y=trainTarget, validation_data=([validData, validShift], validTarget),
        ...
        data_to_predict = [validData, validShift]
        ...
        # TODO:
        if mode_frequency:
            f, t, testDataFreq = signal.stft(testData, 2048, nperseg=stft_nperseg, noverlap=stft_noverlap)
            fs_ind = next(i for i, res in enumerate(f > fs_cap) if res) if fs_cap else None
        ...
        data_to_predict = [testData, testShift]
...
_, _, _ = plot_svr_plot(targets, preds, np.std(targets), 'TEMP SHIFTS', run_id, path_to_save, shifts=np.array(TEMP_SHIFTS).flatten())
    ...
    with open(full_save_path, 'a', newline='') as f:
        ...
        if CONTACT_MODE:
            ...
            data = [
                ...
                num_batches, percentile, contacts_to_use, regions_to_use, stft_nperseg, stft_noverlap
            ]
        else:
            if f.tell() == 0:
                # First time writing to file. Write header row.
